...
function setupInstructionsSheet(ss) {
  ...
  const content = [
    ...
    [''],
    [''],
    ['  LOOKUPS — Interactive per-entity drill-down views'],
    [''],
    ['  • IndividualLookup — What\'s this person\'s device assignment history?'],
    ['    Dropdown-driven view: pick a user in B1, the sheet fetches their full'],
    ['    asset assignment/unassignment history from /users/{userId}/activities.'],
    ['    Works for districts that assign devices directly (no formal checkouts).'],
    ['    Columns: Date, Action, Asset Tag, Serial, Model, Location, Currently With.'],
    [''],
    ['  • VerificationLookup — What\'s this asset\'s audit verification history?'],
    ['    Paste-driven view: type or paste an Asset Tag or Serial Number in B1,'],
    ['    the sheet looks it up in AssetData and fetches verification audit'],
    ['    history from /assets/{assetId}/verifications. Useful for confirming'],
    ['    when and where a device was last verified during inventory audits.'],
    ['    Columns: Date, Result, Method, Location, Room, Verified By, Comments.'],
    [''],
    [''],
    ...
    [''],
    ['  Lookups'],
    ['    • IndividualLookup, VerificationLookup (optional — install individually)'],
    ['    • Regenerate Lookups — Refresh all installed sheets in this category'],
    [''],
    ...
    [''],
    ['═══════════════════════════════════════════════════════════════════════════════'],
    ['BUILT-IN ASSET DASHBOARD (WEB APP)'],
    ['═══════════════════════════════════════════════════════════════════════════════'],
    [''],
    ['A native Apps Script Web App ships with this project: a shareable URL that'],
    ['renders a full-page, tabbed dashboard powered by your AssetData and analytics'],
    ['sheets. Each district deploys their own instance bound to their sheet.'],
    [''],
    ['ONE-TIME DEPLOYMENT:'],
    ['1. Open Extensions → Apps Script from this sheet.'],
    ['2. Click Deploy → New deployment. Type: Web app.'],
    ['3. Execute as: Me (the deployer).'],
    ['   *Critical — viewers won\'t need spreadsheet access this way.*'],
    ['4. Who has access: Anyone within your domain (recommended).'],
    ['5. Click Deploy, authorize, and copy the /exec URL.'],
    ['6. Paste the URL into the DASHBOARD_URL row in the Config sheet.'],
    ['7. Run iiQ Assets > Setup > Show Dashboard URL to confirm.'],
    [''],
    ['UPDATING AFTER CODE CHANGES:'],
    ['  Use Deploy → Manage deployments → Edit (pencil) → New version.'],
    ['  This publishes new code at the SAME URL — bookmarks keep working.'],
    ['  Do NOT use "New deployment" for updates; it issues a fresh URL.'],
    [''],
    ['WHAT YOU SEE:'],
    ['  • 4 KPI cards (Total Assets, Avg Age, Warranty Active %, Assignment Rate %)'],
    ['  • 5 chart tabs: Composition, Status, Aging, Budget, Service'],
    ['  • 2 interactive lookup tabs (when their source sheets are installed):'],
    ['      Individual — pick a user, see their asset assignment history'],
    ['      Verification — paste an Asset Tag/Serial, see audit verification history'],
    ['  • 19 charts auto-rendered from registered analytics sheets'],
    ['  • Tabs only appear for sheets you\'ve installed'],
    [''],
    [''],
    ['═══════════════════════════════════════════════════════════════════════════════'],
    ['iiQ TELEMETRY'],
